...
import '../../css/entitycollected.scss'
 ...
 //ES5 实现 使用原型式继承
const ECDialog = Object.create(Dialog)
ECDialog.id = '#dialog .entity-collected'
ECDialog._render = function() {
 ...
 * 显示或隐藏二维码弹窗
 */
ECDialog.toggleQr = function(flag) { 
    let action = flag ? 'show' : 'hide';
    $(this.id)[action]()
}
ECDialog._show = function(param){
    this.toggleQr(true)
    Dialog._show.call(this, param)
    $('#dialog .common').hide()
}
ECDialog.hide = function(flag){
    this.toggleQr(false)
